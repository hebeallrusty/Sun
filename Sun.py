...
def SunTime (oDate,location,tOffset):
	#print("--------- start ----------")
	...
	zenith = [-0.01454, -0.10453, -0.20791, -0.30902]
	
	# 1) calculate the day of the year
	
	N1 = math.floor(275 * oDate.month / 9)
	N2 = math.floor((oDate.month + 9) / 12)
	N3 = (1 + math.floor((oDate.year - (4 * math.floor(oDate.year /4 )) + 2) / 3))
	N = N1 - (N2 * N3) + oDate.day - 30
	#print([N1,N2,N3,N])
	
	# 2) convert the longitude to hour value and calculate an approximate time
	#print(location[0],location
	lngHour = location[1] / 15
	#print(lngHour)
	...
	# sinDec * sin(latitude:
	cosHtempn = [i * math.sin(math.radians(location[0])) for i in sinDec]
	# cosDec * cos(latitude):
	cosHtempd = [i * math.cos(math.radians(location[0])) for i in cosDec]
	...
	for i in range(2):
		for j in range (4):
			...
			if cosH[i][j] is None:
				...
				localtime[i][j] = dt.timedelta(hours=T[i][j]+tOffset)
